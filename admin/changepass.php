 ...
 *  File_name: changepass.php(admin page)
 *  Purpose: This will allow the administrator to change his/her password.
 */
?>
...
<?php
	...
	if(isset($_SESSION['username'])) {          //if the user is logged in
		...
		// the script has been called if it has empty login ID or if user hit "Cancel" it just return to index.php(admin page)
		if (empty($id) || isset($_POST['cancel'])) { 
			Header("Location: index.php"); 
			exit; 
		}
		
		// runs this iff the user has hit the "OK" button in ch
		if (isset($_POST['ok'])) {
		...
		<table class=first border="0" width=1090px style="table-layout:fixed">
			...
			<tr align=center>
				...
				<h3 align=center>Chang_Password</h3>
				<?php
				if ( !empty($message) ) {
